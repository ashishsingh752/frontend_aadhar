    ...
    if (fingerprint_img !== "") {
      ...
      try {
        const response = await axios.post(
          "https://backend-aadhar.onrender.com/api/user/login",
          data
        );
        setemailerror("");
        setformstatus("success");
        const bioresponse = VerifyFinger(
          fingerprint_key,
          response.data.user[0].fingerprint_key
        );
        console.log("bio response", bioresponse);
        if (bioresponse.data.Status) {
          tokenstore(response.data.user);
          setstatus("success");
          seterror("");
          nvg("/");
        ...
        setemailerror("This Email is Not Register");
        setformstatus("failed");
      }
    } else {
      seterror("Finger Print is Required");
    }
  };
  return (
    <div className="min-h-screen w-full flex items-center justify-center bg-gradient-to-r from-blue-500 to-teal-500">
      <div className="w-1/3 bg-white border-2 border-gray-200 rounded-md shadow-md p-8 max-w-4xl ">
        ...
        <div className="flex justify-center items-center text-[#0D9276] text-sm">
          Don't Have an account?
          <NavLink to="/register">
            <span className="underline text-blue-600 ml-2">Click Here</span>
          </NavLink>
        </div>
      </div>
    </div>
